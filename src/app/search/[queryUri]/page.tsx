...
export default async function Page({
	...
	return (
		...
		<p>
			<a className="pageChangeButtons" href={`/search/${encodeURIComponent(query)}?page=${pageNum === 1 ? pageNum : pageNum - 1 }`/* this makes it go to page 1 when it is on page one*/}>&lt;- </a>
			<a className="pageChangeButtons" href={`/search/${encodeURIComponent(query)}?page=${pageNum + 1}`}> -&gt;</a>
		</p>
		</>
	)
}
