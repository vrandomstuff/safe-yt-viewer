...
export default async function Page({
	...
	const blacklistEntry = await db
		.select()
		.from(blacklist)
		.where(eq(blacklist.videoId, id));
	const whitelistEntry = await db
		.select()
		.from(whitelist)
		.where(eq(whitelist.videoId, id));
	if (
		(cacheEntry.length === 0 && whitelistEntry.length === 0) ||
		blacklistEntry.length != 0
	) {
		return <h1>Video is not allowed</h1>;
	}
	return (
		<>
			<YTEmbed id={id} />
