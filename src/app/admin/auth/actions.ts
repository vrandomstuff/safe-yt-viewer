...
export async function redirectIfNotAuthed() {
	...
	const tokenDbEntry = await db.select().from(tokens).where(eq(tokens.token, tokenCookie.value))
	if(tokenDbEntry.length != 1) {
		redirect("/admin/auth")
	}
}
export async function logIn(secret: string, secure: boolean, id: string | null): Promise<boolean> {
	if(process.env.SHARED_ADMIN_SECRET === undefined) {
		throw new Error("SHARED_ADMIN_SECRET not found.")
	}
	if(secure && id != null) {
		const expected = await digestMessage(secureAuthStore[id] + process.env.SHARED_ADMIN_SECRET)
